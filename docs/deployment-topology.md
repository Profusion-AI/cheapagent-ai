...
- The `/v1/*` namespace is served by an edge function (`netlify/edge-functions/api-v1.mjs`, bound via `[[edge_functions]]` in `netlify.toml`). It answers the pre-gate stub: missing/malformed key → `401`, key-shaped header (`^ca_(live|test)_[A-Za-z0-9]{32,}$`) → `501`, both with `docs_url` + `early_access_url` nested under `error` (per the published `openapi.yaml` Error schema). It executes nothing, reads no Blobs, and counts nothing — it proves DNS/SSL/routing and the key-header contract only.
